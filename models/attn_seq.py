...
from .common import TOTAL_DIM, NUM_DIFF_STEPS, train_oneshot_common, diffusion_step_common, categorical_diffuse, betas
...
class DiffLayer(torch.nn.Module):
    def __init__(self, hidden_dim, rank=16, attn_impl="flash"):  # "flash" or "xformers" or "naive"
        ...
        self.proj_q_seq = torch.nn.Linear(hidden_dim, qkv_dim, bias=False)
        self.proj_k_seq = torch.nn.Linear(hidden_dim, qkv_dim, bias=False)
        self.proj_v_seq = torch.nn.Linear(hidden_dim, qkv_dim, bias=False)
        self.proj_out_seq = torch.nn.Linear(qkv_dim, hidden_dim, bias=False)
        self.ln_attn_seq = torch.nn.LayerNorm(hidden_dim)
    ...
    def forward(self, sequence, node, chemical_environment):        # x: (B,L,H)
        ...
        Qs, Ks, Vs = self.proj_q_seq(sequence), self.proj_k_seq(sequence), self.proj_v_seq(sequence)
        if self.attn_impl == "flash":
            context = self._flash_attn(Qs, Ks, Vs)
        elif self.attn_impl == "xformers":
            context = self._xformers_attn(Qs, Ks, Vs)
        elif self.attn_impl == "naive":
            context = self._naive_attn(Qs, Ks, Vs)
        if self.attn_impl is not None:
            extracted_information = self.ln_attn_seq(sequence + self.proj_out_seq(context))
        sequence = sequence + extracted_information
...
class diffmodel(torch.nn.Module):
    def __init__(self, orig_model, maxlen=512, k_neighbors=32, EXV_dim=384):
        ...
        '''
        # Integrate (num_states, L, k, EXV) -> (num_states, L, H)
        self.integ_chem_env = AttentionIntegrator(seq_idx=2, emb_idx=3, hidden_dim=self.EXV_dim, rank=16)
        # Project EXV to hidden_dim
        self.proj_chem_env = torch.nn.Linear(EXV_dim, self.hidden_dim)
        # Integrate (num_states, L, H) -> (L, H)
        self.integ_state_h_V = AttentionIntegrator(seq_idx=0, emb_idx=2, hidden_dim=self.hidden_dim, rank=16)
        # Integrate (num_states, L, H) -> (L, H)
        self.integ_state_chem_env = AttentionIntegrator(seq_idx=0, emb_idx=2, hidden_dim=self.hidden_dim, rank=16)

        self.seq_enc_layer = torch.nn.Embedding(TOTAL_DIM, self.hidden_dim)

        self.decoder_layers = torch.nn.ModuleList(
            [DiffLayer(self.hidden_dim, rank=16, attn_impl="xformers") for _ in range(6)]
        )
        self.W_out = torch.nn.Linear(self.hidden_dim * 3, 21)
...
def train(model : diffmodel, feature_dict_list, loss_fn, optimizer, diff_levels, device, eval_only=False):
    ...
    beta = betas[diff_levels] 
    keep_mask = torch.bernoulli(
        (1.0 - beta.unsqueeze(1)).expand_as(x_true), generator=gen
    ).bool().to(device)                                 # True -> keep original
    replacement_tokens = torch.full_like(x_true, 20) # 20 = 'X'
    x = torch.where(keep_mask, x_true, replacement_tokens)  # (B, L)
    num_masked = (~keep_mask).sum(dim=1)  # (B,)
    x_tm1 = x.clone()
    self_diff_levels = self_diff_levels.to(device)
    ...
    for i in range(len(diff_levels)):
        ...
        tar_i = S_true[0].long()
        ce_t = loss_fn(logits_i, tar_i)
        # Mask out the loss for unmasked tokens if the loss is under 0.1
        loss_masked = torch.where((ce_t < 0.1) & keep_mask[i], torch.zeros_like(ce_t), ce_t)
        loss += loss_masked.sum() / (num_masked[i] + 1)
