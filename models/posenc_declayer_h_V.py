...
class diffmodel(torch.nn.Module):
    def __init__(self, orig_model, maxlen=512, k_neighbors=32, EXV_dim=384):
        ...
        self.EXV_integration_layer = torch.nn.Linear(self.EXV_dim, self.hidden_dim)
        self.graph_integration_layer = DecLayer(
            self.hidden_dim, 
            self.hidden_dim, 
            dropout=0.1
        )
    ...
    def integrate_embeddings(self, h_V, h_EXV):
        ...
        h_EXV = self.EXV_integration_layer(h_EXV)
        h_V = self.graph_integration_layer(h_V, h_EXV)
    ...
    def trainable_parameters(self):
        ...
        trainable_params += self.EXV_integration_layer.parameters()
...
def train_oneshot(model : diffmodel, feature_dict_list, loss_fn, optimizer, diff_levels, device, eval_only=False):
    ...
    embeddings = model.integrate_embeddings(h_V.detach(), h_EXV_encoder_fw.detach())
    
    # Run diffusion step
    logits, x_tm1 = diffusion_step_common(model, embeddings, diff_levels, S_true, device, use_pos_enc=True)
