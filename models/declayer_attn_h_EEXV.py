...
class diffmodel(torch.nn.Module):
    ...
    def integrate_embeddings(self, h_EXV, h_E):
        ...
        h_V = self.graph_integration_layer(h_EXV, h_E)
...
def train_oneshot(model : diffmodel, feature_dict_list, loss_fn, optimizer, diff_levels, device, eval_only = False):
    ...
    embeddings = model.integrate_embeddings(h_EXV_encoder_fw, h_E)
    
    self_logits, x_tm1 = diffusion_step_common(model, embeddings, diff_levels, S_true, device)
