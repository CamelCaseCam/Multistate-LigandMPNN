...
from .common import TOTAL_DIM, NUM_DIFF_STEPS, train_oneshot_common, diffusion_step_common, categorical_diffuse
...
class DiffLayer(torch.nn.Module):
    def __init__(self, hidden_dim, rank=16, attn_impl="flash"):  # "flash" or "xformers" or "naive"
        ...
        self.norm_seq = torch.nn.LayerNorm(hidden_dim)
        self.norm_node = torch.nn.LayerNorm(hidden_dim)
        self.norm_chem_env = torch.nn.LayerNorm(hidden_dim)
    ...
    def forward(self, sequence, node, chemical_environment):        # x: (B,L,H)
        ...
        sequence = self.seq_integration_layer(context_stacked)

        # 2. Generate new node tensor
        context_stacked = torch.cat([sequence, node], dim=-1)
        node = self.node_gen_layer(context_stacked)
        ...
        chemical_environment = self.chem_env_layer(context_stacked)

        # Norm can either be applied before or after integration, but doing it after preserves more information
        sequence = self.norm_seq(sequence)
        sequence = self.dropout(sequence)
        node = self.norm_node(node)
        node = self.dropout(node)
        chemical_environment = self.norm_chem_env(chemical_environment)
        chemical_environment = self.dropout(chemical_environment)
...
class diffmodel(torch.nn.Module):
    def __init__(self, orig_model, maxlen=512, k_neighbors=32, EXV_dim=384):
        ...
        '''
        self.comb_weights_h_V = torch.nn.Linear(self.hidden_dim * 2, self.hidden_dim)
        self.comb_weights_h_EXV = torch.nn.Linear(self.hidden_dim * 2, self.hidden_dim)
    ...
    def integrate_embeddings(self, h_V, h_EXV):
        ...
        for state_idx in range(1, h_V.shape[0]):
            ...
            ratio[state_idx] = self.comb_weights_h_V(inp)
        # Now, use the ratio to combine the embeddings (summing to 1)
        ratio = ratio / (torch.sum(ratio, dim=-1, keepdim=True) + 1e-8)
        # Summing time!
        h_V_int = torch.sum(ratio * h_V, dim=0, keepdim=True)
        
        # Combine the h_EXV embeddings
        ...
        ratio = torch.zeros_like(h_EXV)
        for state_idx in range(1, h_EXV.shape[0]):
            last = h_EXV[state_idx - 1]
            current = h_EXV[state_idx]
            inp = torch.cat([last, current], dim=-1)
            ratio[state_idx] = self.comb_weights_h_EXV(inp)
        # Now, use the ratio to combine the embeddings (summing to 1)
        ratio = ratio / (torch.sum(ratio, dim=-1, keepdim=True) + 1e-8)
        # Summing time!
        h_EXV_int = torch.sum(ratio * h_EXV, dim=0, keepdim=True)

        return h_V_int, h_EXV_int

    
    def trainable_parameters(self):
        trainable_params = []
        trainable_params += self.comb_weights_h_V.parameters()
        trainable_params += self.comb_weights_h_EXV.parameters()
