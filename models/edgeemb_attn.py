...
class diffmodel(torch.nn.Module):
    def __init__(self, orig_model, hidden_dim=256, maxlen=512, k_neighbors=32, EXV_dim=384):
        ...
        # Add edge index embedding layer
        self.edge_idx_embed = torch.nn.Embedding(maxlen, self.emb_hidden_dim)
        self.integ_edge_embed = torch.nn.Linear(self.emb_hidden_dim * 2, self.emb_hidden_dim)
        ...
        self.decoder_layers = torch.nn.ModuleList(
            [DiffLayer(self.emb_hidden_dim, rank=16, attn_impl="xformers") for _ in range(6)]
        )
    ...
    def integrate_embeddings(self, h_V, h_EXV, E_idx=None):
        """
        Integrate embeddings with edge index information
        
        Args:
            h_V: Node embeddings (num_states, L, H)
            h_EXV: Chemical environment embeddings (num_states, L, k, H)
            E_idx: Edge indices (num_states, L, k)
        
        Returns:
            h_V_int: Integrated node embeddings (1, L, H)
            chemical_env_int: Integrated chemical environment embeddings (1, L, H)
        """
        # Combine the h_V embeddings
        h_V_proj = self.proj_h_V(h_V)  # (num_states, L, H)
        
        # If edge indices are provided, add edge embeddings to nodes
        if E_idx is not None:
            # Get shape information
            num_states, L, k = E_idx.shape
            
            # Embed edge indices
            edge_embeddings = self.edge_idx_embed(E_idx)  # (num_states, L, k, H)
            
            # Sum across neighbor dimension
            edge_sum = edge_embeddings.sum(dim=2)  # (num_states, L, H)
            
            # Add to node embeddings
            h_V_proj = self.integ_edge_embed(torch.cat([h_V_proj, edge_sum], dim=-1))  # (num_states, L, H)
        
        h_V_int = self.integ_state_h_V(h_V_proj, return_weights=False).unsqueeze(0)  # (1, L, H)
    ...
    def trainable_parameters(self):
        trainable_params = []
        trainable_params += self.proj_h_V.parameters()
        trainable_params += self.integ_chem_env.parameters()
        trainable_params += self.proj_chem_env.parameters()
        ...
        trainable_params += self.edge_idx_embed.parameters()  # Add edge embedding parameters
        trainable_params += self.integ_edge_embed.parameters()
        trainable_params += self.init_layer.parameters()
...
def train_oneshot(model : diffmodel, feature_dict_list, loss_fn, optimizer, diff_levels, device, eval_only=False):
    ...
    # Get embeddings using h_V processed with h_EXV, now including E_idx
    h_V, h_EXV = model.integrate_embeddings(h_V.detach(), h_EXV_encoder_fw.detach(), E_idx.detach())
    ...
    # Calculate loss
    step_losses = []  # List to hold individual losses for each diff_level
    total_loss = torch.tensor(0.0, device=device)
    for i in range(len(diff_levels)):
        logits_i = logits[i]
        tar_i = S_true[0].long()
        ce_t = loss_fn(logits_i, tar_i)
        step_loss = ce_t.mean().item()  # Per-step loss
        step_losses.append(step_loss)
        total_loss += ce_t.mean()   # We can't use `.item` here because it disrupts gradient flow
    
    if not eval_only:
        (total_loss / len(diff_levels)).backward()  # Average for backward compatibility
        optimizer.step()
    
    output_dict["loss"] = total_loss / len(diff_levels)  # Existing aggregated loss
    output_dict["step_losses"] = step_losses  # NEW: Return per-step losses
    return output_dict
...
def train(model: diffmodel, feature_dict_list, loss_fn, optimizer, diff_levels, device, eval_only=False):
    ...
    # NEW: Instead of accumulating into 'loss', collect per-step losses
    step_losses = []  # List to hold individual losses for each diff_level
    total_loss = torch.tensor(0.0, device=device)
    for i in range(len(diff_levels)):
        logits_i = logits[i]
        ...
        if num_masked > 0:
            masked_loss_value = masked_loss.sum() / num_masked
        else:
            masked_loss_value = 0.0
        step_losses.append(masked_loss_value.item())
        total_loss += masked_loss_value
    
    total_loss = total_loss / len(diff_levels)  # Average for existing logic
    
    if not eval_only:
        total_loss.backward()
        optimizer.step()
    
    output_dict["loss"] = total_loss.item()  # Existing aggregated loss
    output_dict["step_losses"] = step_losses  # NEW: Return per-step losses
    return output_dict